  ...
  <div class="tabbar">
    <div class="wrapper" ref="wrapper" :style="wrapperStyle">
      <div class="bscroll-container" >
        ...
        <ul class="content">
          <li v-for="(item,index) in data" @click="handleInfo(item)" class="li_content">
            <div class="img_context">
				<!-- <div v-for="(tItem,tIndex) in tagList" 
				:style="{position: 'absolute',width: tagWidth+'px',top: tItem.yAxes+'%',left: tItem.xAxes+'%'}" v-if="item.modityId==tItem.modityId">
					<img :src="tItem.url" style="width: 100%;display: block;box-shadow: none;">
				</div> -->
              <div v-for="(tItem,tIndex) in item.tagList" :style="{position: 'absolute',width: clientWidth*tItem.percentage/100+'px',
              top: tItem.top>100?'':tItem.top+'%',left: tItem.left>100?'':tItem.left+'%',bottom: tItem.top>100?0:'',right: tItem.left>100?0:''}" v-if="item.tagList.length">
                <img :src="tItem.url" style="width: 100%;display: block;box-shadow: none;">
              </div>
              <img @load="imageLoad" :src="getImgLocalUrl(item.mainPicture+'?x-oss-process=image/resize,m_pad,h_1000,w_1000/quality,q_70')" alt="" class="product_img">
...
export default {
  data() {
    return {
      storeId: this.$store.getters.loginUser.orgId,
      x: 0,
      y: 0,
      hasNextPage: true,
  ...
  mounted() {
    ...
    this.wrapperHeight =
      screenHeight - titleHeight - navHeight - siderPaddingHeight - cardHeight; //减去页面上固定高度height

    window.onresize = function() {
      _self.clientWidth = document.getElementsByClassName(
        "product_img"
      )[0].clientWidth;
      // _self.tagWidth = width/6;
    };
    setTimeout(() => {
      this._initScroll();
    }, 500);
...
this.scroll.refresh();
    ...
    getTags() {
      this.$http({
        method: "get",
        url: "modity/modityTags?storeId=" + this.storeId
        ...
        .then(response => {
          if (response.data.code == 200) {
            ...
            if (this.scrollFlag == true) {
              if (response.data.data.list) {
                for (
                  let i = 0, len = response.data.data.list.length;
                  i < len;
                  i++
                ) {
                  response.data.data.list[i].tagList = [];
                  for (let j = 0, leng = this.tagList.length; j < leng; j++) {
                    if (
                      response.data.data.list[i].modityId ==
                      this.tagList[j].modityId
                    ) {
                      let percentage = this.tagList[j].percentage
                        ? this.tagList[j].percentage
                        : 17;
                      let obj = {
                        url: this.tagList[j].url,
                        left: this.tagList[j].xAxes,
                        top: this.tagList[j].yAxes,
                        percentage: percentage
                      };
                      response.data.data.list[i].tagList.push(obj);
                    }
                  }
                }
            ...
            setTimeout(() => {
              ...
              // this.tagWidth = this.clientWidth/6;
           
              let img_contextHeight = $(".img_context").height();
           
              $(".product_img").css("height", img_contextHeight);
            }, 200);
