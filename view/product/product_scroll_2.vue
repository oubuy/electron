        ...
        <ul class="content">
          <li v-for="item in data" @click="handleInfo(item)">
            <div class="img_context">
              <img :src="getImgLocalUrl(item.mainPicture+'?x-oss-process=image/resize,m_pad,h_1000,w_1000/quality,q_70')" alt="">

             <!-- <img src="~@/assets/local/images/1-2.png" :style="{left:positionLeft +'%',top:positionTop +'%'}" class="label_btn"> -->
             <img v-if="item.labelData" :src="item.labelData.url" :style="{left:item.labelData.xAxes +'%',top:item.labelData.yAxes +'%'}" class="label_btn">
...
export default {
  ...
  methods: {
            ...
            if (this.scrollFlag == true) {
              if (response.data.data.list) {
                ...
                if (this.data && this.data.length != 0) {
                  ...
                  let temp=this.handleAddLabel(response.data.data.list, this.labelData);
                     this.data = this.data.concat(temp);
                } else {
                  //   this.data = response.data.data.list;
                  this.data=this.handleAddLabel(response.data.data.list, this.labelData);
                }
                this.noDataFlag =
              ...
              if (this.formData.pageNum == 1) {
                if (response.data.data.list.length > 0) {
                  ...
                  //   this.data = response.data.data.list;
                  this.handleGetLabel(response.data.data.list);
                  console.log(this.data, "第一页---this.data");
                } else {
                  this.data = null;
    ...
    handleGetLabel(list_data) {
      //获取标签值
      this.loginUserInfo = this.$store.getters.loginUser;
      let params = {
        // storeId:this.loginUserInfo.orgId
        storeId: 4498
      };
      this.$http({
        method: "get",
        url: "modity/modityTags",
        params: params
      }).then(response => {
      
        if (response.data.code == 200) {
          this.initLabelFlag = false;
          let label_data = response.data.data;
          ...
          if (this.formData.pageNum == 1) {
            this.data = this.handleAddLabel(list_data, this.labelData);
          }
          
        }
      });
    },
    handleAddLabel(listData, listLabel) {
     
      for (var i = 0; i < listData.length; i++) {
        for (var j = 0; j < listLabel.length; j++) {
          if (listData[i].modityId == listLabel[j].modityId) {
            listData[i].labelData = listLabel[j];
            break;
          }
        }
      }
